...
themselves, for example if it is a bug happening on a platform you don't have access to. And even when a bug requires actively
...
If an issue looks hard to replicate triagers might even chose not to try to test it without an MRP or more details to avoid wasting
time on an issue they are unlikely to be able to replicate in the first place.
